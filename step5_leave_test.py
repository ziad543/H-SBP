...
import json # Added to load config early
...
NODE_LOG_DIR_ABS = "/mnt/workarea/H-SBP/logs" # ABSOLUTE path INSIDE node

# *** Define ONLY the nodes that will be physically created and run scripts ***
NODE_IDS = { # Mapping friendly name to config ID
    "SL": "sl-0",
    "CH1": "ch-1",
    "CH2": "ch-2",
    "M104": "m-104", # Joining C1
    "M204": "m-204"  # Joining C2
}

# *** Define target IPs ONLY for the nodes being created ***
# *** Adjust Node IDs based on creation order (1 to 5) ***
NODE_TARGET_IPS = {
     1: "172.16.0.1", # SL
     2: "172.16.0.2", # CH1
     3: "172.16.0.3", # CH2
...
def get_node_ip_via_cmd(core: client.CoreGrpcClient, session_id: int, node_id: int, iface_name: str = "eth0"):
    command = f"ip -4 addr show {iface_name}"; logging.debug(f"Executing command on node {node_id}: {command}")
    try:
...
def main():
    ...
    core = client.CoreGrpcClient(); session = None; nodes = {}

    try:
        core.connect(); logging.info("Connected to CORE gRPC")
        session = core.create_session(); logging.info(f"Created CORE session {session.id}")

        emane_position = Position(x=300, y=300); emane_node = session.add_node(100, _type=NodeType.EMANE, position=emane_position, emane=EmaneRfPipeModel.name); logging.info(f"Added EMANE node {emane_node.id}...")

        # Node Positions - Define positions for the nodes we are creating
        positions = {
            "SL": Position(x=300, y=100), "CH1": Position(x=150, y=250), "CH2": Position(x=450, y=250),
            "M104": Position(x=150, y=410), # Joining C1
            "M204": Position(x=450, y=410)  # Joining C2
        }

        # *** Create ONLY SL, CHs, and Joining Members ***
        node_id_counter = 1
        nodes_to_create = ["SL", "CH1", "CH2", "M104", "M204"] # Explicit list
        ...
        # Start Session
        core.start_session(session); logging.info("Session started.")
        wait_time = 5; logging.info(f"Waiting {wait_time} seconds for network initialization...")
        time.sleep(wait_time)

        # Get/Verify ALL Node IPs (for the nodes we actually created)
        node_ips = {}; all_ips_found = True; logging.info("Verifying ALL Node IPs via node_command...")
        for name_key in nodes_to_create:
            if name_key not in nodes: continue # Should not happen if creation loop worked
            ip = get_node_ip_via_cmd(core, session.id, nodes[name_key].id, "eth0"); expected_ip = NODE_TARGET_IPS.get(nodes[name_key].id)
        ...
        # --- Start Initial Node Scripts (SL, CH1, CH2 ONLY) ---
        logging.info(f"\n--- Starting SL and CH Scripts (output redirected to {NODE_LOG_DIR_ABS}/*.log) ---")
        sl_script = os.path.join(HSBP_SCRIPT_DIR, "swarm_leader.py"); sl_log = os.path.join(NODE_LOG_DIR_ABS, f"{NODE_IDS['SL']}.log"); sl_cmd = f"{PYTHON_EXEC} -u {sl_script} --id {NODE_IDS['SL']} --config {CONFIG_FILE_PATH} > {sl_log} 2>&1"
        logging.info(f"Executing SL on node {nodes['SL'].id}: {sl_cmd}"); core.node_command(session.id, nodes["SL"].id, sl_cmd, wait=False, shell=True); time.sleep(2)
        ch1_script = os.path.join(HSBP_SCRIPT_DIR, "cluster_head.py"); ch1_log = os.path.join(NODE_LOG_DIR_ABS, f"{NODE_IDS['CH1']}.log"); ch1_cmd = f"{PYTHON_EXEC} -u {ch1_script} --id {NODE_IDS['CH1']} --config {CONFIG_FILE_PATH} --sl-ip {node_ips['SL']} > {ch1_log} 2>&1"
        logging.info(f"Executing CH1 on node {nodes['CH1'].id}: {ch1_cmd}"); core.node_command(session.id, nodes["CH1"].id, ch1_cmd, wait=False, shell=True); time.sleep(1)
        ch2_script = os.path.join(HSBP_SCRIPT_DIR, "cluster_head.py"); ch2_log = os.path.join(NODE_LOG_DIR_ABS, f"{NODE_IDS['CH2']}.log"); ch2_cmd = f"{PYTHON_EXEC} -u {ch2_script} --id {NODE_IDS['CH2']} --config {CONFIG_FILE_PATH} --sl-ip {node_ips['SL']} > {ch2_log} 2>&1"
        logging.info(f"Executing CH2 on node {nodes['CH2'].id}: {ch2_cmd}"); core.node_command(session.id, nodes["CH2"].id, ch2_cmd, wait=False, shell=True)
        

        logging.info("\n--- SL/CHs running. Waiting 15s for CH internal initialization and inter-CH key exchange ---")
        time.sleep(5) # Wait for CHs to initialize M0 state and connect to SL

        # --- Start Joining Member Scripts (M104, M204 ONLY) ---
        logging.info("\n--- Starting Single Joining Member Script(s) ---")
        # Load config again just to get joining member IDs correctly mapped
        try:
            with open(CONFIG_FILE_PATH, 'r') as f: config_data = json.load(f) # Read config on host
        ...
        joining_member_key_map = {} # Map config ID back to friendly key (e.g., "m-104" -> "M104")
        if cluster1_joining_id: joining_member_key_map[cluster1_joining_id] = "M104"
        ...
        for joining_config_id, name_key in joining_member_key_map.items():
            ...
            member_script = os.path.join(HSBP_SCRIPT_DIR, "member.py"); member_log = os.path.join(NODE_LOG_DIR_ABS, f"{joining_config_id}.log")
            # Determine correct CH IP based on which cluster this joiner belongs to
            ...
            logging.info(f"Executing JOINING {name_key} ({joining_config_id}) on node {nodes[name_key].id}: {member_cmd}"); core.node_command(session.id, nodes[name_key].id, member_cmd, wait=False, shell=True);
            nodes_started_join[name_key] = True; time.sleep(1)
        ...
        time.sleep(2)
        # Inside gRPC script, after single join test
        logging.info("\n--- Triggering Batch Leave Event for CH1 ---")
        ch1_ip_for_trigger = node_ips.get("CH1")
        ch1_control_port_for_trigger = 5100 + 1 -1 # Base + cluster_id (1) -1, assuming config
        num_members_to_leave = 2 # Example
        ...
        if ch1_ip_for_trigger:
            ...
            # Run this command on a node that can reach CH1, e.g., the SL node or the host if CORE networking allows
            # For simplicity, let's assume SL can send it (node nodes["SL"].id)
            core.node_command(session.id, nodes["SL"].id, trigger_cmd, wait=True, shell=True)
        else:
            logging.error("CH1 IP not found, cannot trigger batch leave.")
        ...
        core.close(); logging.info("Disconnected from CORE")
